...
def showVotes(root,frame1):
    ...
    trsLogo = ImageTk.PhotoImage((Image.open("img/trs.jpg")).resize((45,30),Image.ANTIALIAS))
    trsImg = Label(frame1, image=trsLogo).grid(row = 4,column = 0)

    tdpLogo = ImageTk.PhotoImage((Image.open("img/tdp.png")).resize((40,35),Image.ANTIALIAS))
    ...
    Label(frame1, text=" Congress        :          ", font=('Helvetica', 12, 'bold')).grid(row = 3, column = 1)
    Label(frame1, text=result['congress'], font=('Helvetica', 12, 'bold')).grid(row = 3, column = 2)

    Label(frame1, text=" BRS             :          ", font=('Helvetica', 12, 'bold')).grid(row = 4, column = 1)
    Label(frame1, text=result['trs'], font=('Helvetica', 12, 'bold')).grid(row = 4, column = 2)

    Label(frame1, text=" TDP    :          ", font=('Helvetica', 12, 'bold')).grid(row = 5, column = 1)
    Label(frame1, text=result['tdp'], font=('Helvetica', 12, 'bold')).grid(row = 5, column = 2)
