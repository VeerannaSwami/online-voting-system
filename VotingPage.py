...
def votingPg(root,frame1,client_socket):
    ...
    Radiobutton(frame1, text = "Congress\n\nRahul Gandhi", variable = vote, value = "congress", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"congress",client_socket)).grid(row = 3,column = 1)
    ...
    Radiobutton(frame1, text = "BRS Party\n\nKCR", variable = vote, value = "trs", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"trs",client_socket) ).grid(row = 4,column = 1)
    trsLogo = ImageTk.PhotoImage((Image.open("img/trs.jpg")).resize((55,40),Image.ANTIALIAS))
    trsImg = Label(frame1, image=trsLogo).grid(row = 4,column = 0)

    Radiobutton(frame1, text = "TDP Party\n\nChandra Babu", variable = vote, value = "tdp", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"tdp",client_socket)).grid(row = 5,column = 1)
    tdpLogo = ImageTk.PhotoImage((Image.open("img/tdp.png")).resize((50,45),Image.ANTIALIAS))
    tdpImg = Label(frame1, image=tdpLogo).grid(row = 5,column = 0)
